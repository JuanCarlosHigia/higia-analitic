    ...
    with CPredictions:
        ...
        with st.container(border=True, height= maxHeight):
            ...
            def getTextColor(text):
                ...
                if text == "!Diagnosed" or text == "!Undiagnosed":
                    return red
                else:
                    return green
            ...
            with CDiabetes:
                if diabetesPercentaje > 0:
                    ...
                    text = alt.Chart(diabetesTextDataframe).mark_text(fontSize=30, color=getTextColor(textString), fontWeight="bold").encode(
                        text="text:N"
                    )

                    # color=alt.Color("text:N", legend=None, scale=alt.Scale(range=textPalete))

                    st.altair_chart(diabetes_arc + text, use_container_width=True)

                    #st.altair_chart(diabetes_arc, use_container_width=True)
